...
- Upload LT22 & LX03 at the top of the dashboard
- Compute FEFO tiles, charts, tables, and pick_priority.csv
- AS/400-style look (green-on-black)

Author: rcbt2000-byte + M365 Copilot
"""
import os
...
def find_col(cols, keys):
    """Return first matching column by keyword contains (case-insensitive)."""
    for k in keys:
        for c in cols:
            try:
...
def compute_analysis(lt22_path, lx03_path):
    ...
    # Load LX03 (prefer sheet 1 if multiple sheets exist)
    try:
    ...
    if expd_lt and expd_lt in lt22.columns:
        def bucket(d):
            ...
            return '150+'
        lt22['EXP_BUCKET'] = lt22[expd_lt].apply(bucket)
        lt22_buckets = (lt22.groupby('EXP_BUCKET').size()
                        .reindex(['0-30','31-90','91-150','150+','Unknown']).fillna(0).astype(int)
                        .reset_index(name='Rows'))
    else:
        lt22_buckets = pd.DataFrame()
    ...
    # Pick Priority (Material → SLED → Batch)
    ...
    # Persist summary for the view layer
    results = {
